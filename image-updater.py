...
def image(version, repository, file, chart):
    try:
        ...
        if type(version) and type(repository) == NoneType:
        ...
        if chart:
            if type(version) == str:
                if version != values_file[f"{chart}"]['image']['tag'] :
                    ...
                    print(f"Version {version} has been setted successfully in the chart {chart}")
                elif version == values_file[f"{chart}"]['image']['tag'] :
                    print(f"New version { version } is the same in the values.yaml")
            pass
            if type(repository) == str:
                if repository != values_file[f"{chart}"]['image']['repository']:
                    values_file[f"{chart}"]['image']['repository'] = repository
                    file_saver(file, values_file)
                    print(f"Version {repository} has been setted successfully in the chart {chart}")
                elif  repository == values_file[f"{chart}"]['image']['repository']: 
                    print(f"New repository { repository } is the same in the values.yaml for the chart {chart}")
            pass
        if not chart:
            if type(version) == str:
...
@click.option('-v','--version', help='New version to set in values.yaml', required=True)
@click.option('-f','--file', help='Path for values.yaml', required=True)
@click.option('--chart', help='Chart to replace tag')
@click.command()
...
@click.option('-r','--repository', help='New repository to set in values.yaml', required=True)
@click.option('-f','--file', help='Path for values.yaml', required=True)
@click.option('--chart', help='Chart to replace tag')
@click.command()
